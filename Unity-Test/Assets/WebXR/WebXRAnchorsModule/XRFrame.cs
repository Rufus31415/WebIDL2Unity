...
public partial  class XRFrame : WebIDL2UnityObject {
    ...
    public delegate void XRFrame_CreateAnchor_delegate(XRAnchor a0);

    private delegate void XRFrame_CreateAnchor_delegate_native(int promise, int a0);

    [AOT.MonoPInvokeCallback(typeof(XRFrame_CreateAnchor_delegate_native))]
    private static void XRFrame_CreateAnchor_promise(int promise, int a0)
    {
        XRFrame_CreateAnchor_promises[promise].AfterCallback(new object[] { a0 == 0 ? null : new XRAnchor(a0) });
    }
